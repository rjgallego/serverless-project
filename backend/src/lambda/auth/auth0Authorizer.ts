...
const jwksUrl = 'https://dev-jor6ny5m.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  if(header.alg !== 'RS256'){
    throw new JsonWebTokenError('Only RS256 algorithm supported')
  }

  const cert = await getCertificate(jwksUrl)
  

  verify(token, cert, {algorithms: ['RS256']} ) as JwtPayload
